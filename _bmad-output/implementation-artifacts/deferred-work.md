...
## Deferred from: code review of 1-3-provisioned-only-enforcement (2026-07-16)

- **Tombstoned shop rows remain usable through the auth adapter** — `ClerkAuthAdapter.getCurrentShop()` filters only by `clerk_user_id`, while the shop repository filters `deleted_at`; a tombstoned shop owner can therefore continue through authenticated shop-owner routes. This predates the Story 1.3 baseline (`e9aa0d1`) and should be fixed in the auth/offboarding work that introduces shop tombstoning. [adapters/clerk/auth.ts:29-40]
